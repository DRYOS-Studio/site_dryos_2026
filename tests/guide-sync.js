// G4: os comandos (blocos de código), o deep link de instalação e o prompt de teste da página são os
// mesmos do README do plugin.
...
(async () => {
  ...
  const pageCmds = new Set([...html.matchAll(/<code data-cmd>([\s\S]*?)<\/code>/g), ...html.matchAll(/<a [^>]*href="([^"]+)"[^>]*data-install/g)].map(m => dec(m[1]).trim()));
  ...
  let bad = 0;
  if (![...mdCmds].some(c => c.startsWith('vscode://anthropic.claude-code/install-plugin?'))) { console.log('G4 FAIL: README sem o deep link de instalação'); bad++; }
